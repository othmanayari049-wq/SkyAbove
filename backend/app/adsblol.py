...
class AdsbLolClient:
    def __init__(self, settings: Settings, client: httpx.AsyncClient | None = None) -> None:
        ...
        self.client = client or httpx.AsyncClient(
            ...
            headers={"User-Agent": "SkyAbove/0.2 (+https://github.com/othmanayari049-wq/SkyAbove)"},
        )
    ...
    async def fetch_nearby(self, lat: float, lon: float, radius_km: float) -> AdsbLolPayload:
        ...
        if not isinstance(payload, dict):
            raise AdsbLolError("ADSB.lol returned an unexpected response shape")

        aircraft_raw = payload.get("ac") or []
        if not isinstance(aircraft_raw, list):
            raise AdsbLolError("ADSB.lol returned an invalid aircraft collection")
        ...
        return AdsbLolPayload(source_time=source_time, aircraft=aircraft)
