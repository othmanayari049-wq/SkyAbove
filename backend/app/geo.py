...
def initial_bearing_deg(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    """Return initial great-circle bearing in degrees clockwise from true north."""
    phi1, phi2 = radians(lat1), radians(lat2)
    d_lambda = radians(lon2 - lon1)
...
def radius_to_bounding_boxes(
    lat: float, lon: float, radius_km: float
) -> list[tuple[float, float, float, float]]:
    """Return OpenSky-compatible bounding boxes: (lamin, lomin, lamax, lomax).

    The function splits a box at the anti-meridian when necessary.
    """
    ...
    if raw_min_lon < -180.0:
        wrapped_min = raw_min_lon + 360.0
        return [
            (min_lat, -180.0, max_lat, raw_max_lon),
            (min_lat, wrapped_min, max_lat, 180.0),
        ]

    wrapped_max = raw_max_lon - 360.0
    return [
        (min_lat, raw_min_lon, max_lat, 180.0),
        (min_lat, -180.0, max_lat, wrapped_max),
    ]
