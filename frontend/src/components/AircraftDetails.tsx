  ...
  if (!aircraft) {
    return (
      <section className="details-panel empty-details">
        ...
        <p>Choose a marker or list item to inspect its latest available state vector.</p>
      </section>
    );
  }

  const verticalIcon =
    (aircraft.vertical_rate_mps ?? 0) >= 0 ? (
  ...
  return (
    <section className="details-panel">
      <div className="details-heading">
        <div>
          <span className="eyebrow">Selected aircraft</span>
          <h2>{aircraft.callsign || aircraft.icao24.toUpperCase()}</h2>
          <p>{aircraft.origin_country || "Origin country unavailable"}</p>
        </div>
        {aircraft.overhead_candidate && <span className="overhead-badge">Overhead candidate</span>}
      </div>
      <div className="details-grid">
        ...
        <Detail icon={<Globe2 size={16} />} label="Data provider" value={aircraft.data_provider || "—"} />
      </div>
      <div className="details-footer">
        <span>ICAO24 <strong>{aircraft.icao24.toUpperCase()}</strong></span>
        <span>Squawk <strong>{aircraft.squawk || "—"}</strong></span>
        <span>Position age <strong>{ageLabel(aircraft.last_contact)}</strong></span>
      </div>
    </section>
  );
}
